"""Deterministic agent stub. Same input/output shape as a real LLM."""
from app.agent.schema import AgentContext, AgentDecision

INSUFFICIENT_FUNDS = {"insufficient_funds", "BAD_REQUEST_INSUFFICIENT_FUNDS"}
INVALID_INSTRUMENT = {"invalid_card_number", "invalid_expiry", "invalid_cvv",
                      "invalid_account_number", "invalid_vpa",
                      "card_expired", "payment_method_disabled"}
FRAUD = {"payment_fraud", "risk_check_failed"}
AUTH_FAIL = {"payment_failed", "authentication_failed",
...
def _diagnose(ctx: AgentContext) -> tuple[str, str]:
    ...
    text = (ctx.failure_reason or "").lower()

    if rcode in INSUFFICIENT_FUNDS or "insufficient" in text:
        return "insufficient_funds", "Liquidity issue on the customer side."
    if rcode in INVALID_INSTRUMENT or "invalid" in text or "expired" in text:
        return "invalid_instrument", "Payment instrument itself is invalid or expired."
    if rcode in FRAUD or src == "risk":
        return "risk_declined", "Declined for fraud / risk reasons."
    if code == "GATEWAY_ERROR" or src == "gateway" or "timeout" in text:
    ...
    if rcode in AUTH_FAIL or step == "payment_authentication":
...
def decide(ctx: AgentContext) -> AgentDecision:
    ...
    # attempts >= 3 escalates everything except invalid_instrument
    # (a fresh link is cheap and the only viable action)
    if ctx.attempt_number >= 3 and diag != "invalid_instrument":
        return AgentDecision(
            diagnosis=diag, confidence=0.75, recommended_action="escalate",
    ...
    if diag == "invalid_instrument":
        return AgentDecision(
            diagnosis=diag, confidence=0.95, recommended_action="payment_link",
    ...
    if diag == "risk_declined":
        return AgentDecision(
            diagnosis=diag, confidence=0.85, recommended_action="escalate",
    ...
    if diag == "authentication_failed":
        return AgentDecision(
            diagnosis=diag, confidence=0.8, recommended_action="payment_link",
            reason=f"{why} Fresh link gives them a clean auth attempt.",
        )

    if diag == "customer_action_required":
        return AgentDecision(
            diagnosis=diag, confidence=0.6, recommended_action="payment_link", reason=why,
        )

    return AgentDecision(
        diagnosis=diag, confidence=0.5, recommended_action="payment_link",
